...
import logging
from gitlab_app.feishu import FeiShu
...
class MR(object):
    ...
    def parse_webhook(self, item: dict):
        ...
        try:
            ...
            logging.error("push user with error:" + str(e))
        if "robot" in push_user:
            push_user.remove("robot")
        if "cr_report" in push_user:
            push_user.remove("cr_report")
        logging.debug("author:" + str(author))
        logging.debug("push-user:" + str(push_user))
        logging.debug("push_data:" + str(push_data))
